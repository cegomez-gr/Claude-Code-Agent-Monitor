...
| `routes/webhooks.js`      | HTTP surface for universal webhook targets: target CRUD (`GET/POST /api/webhooks`, `PATCH/DELETE /api/webhooks/:id` — `type` is immutable after creation), a synchronous test probe (`POST /api/webhooks/:id/test` — always 200, the `ok` flag carries the downstream delivery result), and a per-target delivery log (`GET /api/webhooks/:id/deliveries`). **Security**: target URLs are masked (host + last 4 chars) and secret / custom-header values are redacted in every response — the full URL and signing secret are stored server-side and never leave the server. PATCH uses "set-flag" semantics so omitting `url` / `secret` / `headers` leaves them unchanged. Every mutation calls `invalidateWebhookCache()` |
| `lib/webhooks.js`         | Universal webhook delivery engine. Four target types with per-platform payload formatters: `slack` (header/section/context blocks), `discord` (rich embed), `teams` (legacy O365 MessageCard), and `generic` (stable `{ event, alert }` JSON envelope, supports optional HMAC-SHA256 body signing via `X-Webhook-Signature` + `X-Webhook-Timestamp` and arbitrary custom headers). `dispatchAlert()` fans a fired alert out to every enabled, in-scope target (optional per-rule scoping via `rule_ids`); each `deliver()` POSTs with an `AbortController` timeout and bounded retry/backoff (retries transport errors / 429 / 5xx, never other 4xx) and records the attempt-chain outcome in `webhook_deliveries` (pruned to the newest 2000 rows). Delivery is detached and fully fail-safe — it never throws into the alert path. Enabled targets are cached like alert rules; tunables (`WEBHOOK_TIMEOUT_MS`, `WEBHOOK_MAX_ATTEMPTS`, `WEBHOOK_RETRY_BASE_MS`) are env-overridable. `sendTest()` awaits a synthetic delivery for the test endpoint |
...
erDiagram
    ...
    webhook_targets {
        ...
        TEXT type "slack|discord|teams|generic"
        TEXT url "Full endpoint URL — server-only, masked in API responses"
        INTEGER enabled "1|0"
        TEXT secret "Generic-only HMAC-SHA256 signing secret or NULL"
        TEXT headers "Generic-only extra request headers (JSON) or NULL"
